...
export const GlobalNavbar: React.FC = () => {
  ...
  // Safety: if auth check hangs for more than 3 seconds, stop showing
  // the loading spinner and fall through to the sign-in button.
  ...
  const showLoading = isPending && !authTimedOut;
        ...
        <div className="flex items-center gap-2.5">
          ...
          {showLoading ? (
              ...
              <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-[#131926] px-2.5 py-1 text-xs text-slate-200">
                {session!.user.image ? (
                  <Image
                    ...
                    width={22}
                    height={22}
                    className="h-[22px] w-[22px] rounded-full object-cover ring-1 ring-cyan-400/40"
                    unoptimized
                  />
                ) : (
                  <div className="flex h-[22px] w-[22px] items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-cyan-500 text-[10px] font-bold text-white">
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-indigo-500 via-sky-500 to-cyan-400 px-3 sm:px-4 py-1.5 sm:py-2 text-xs font-semibold text-white shadow-[0_0_15px_rgba(6,182,212,0.3)] transition-all hover:opacity-95 active:scale-95 disabled:opacity-50"
